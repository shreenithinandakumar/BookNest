...
const Cart = () => {
  ...
  useEffect(()=>{
    const fetch = async () =>{
      const res = await axios.get("https://booknest-c5hx.onrender.com/api/v1/get-user-cart", {headers})
  ...
  const deleteItem = async(bookid) =>{
    ...
    const response = await axios.put("https://booknest-c5hx.onrender.com/api/v1/delete-from-cart", {}, {headers})
  ...
  const PlaceOrder = async() => {
    try{
      const response = await axios.post(`https://booknest-c5hx.onrender.com/api/v1/place-order`, {order: Cart}, {headers})
      alert(response.data.message)
  ...
  <div className="bg-zinc-900 px-12 h-screen py-8 ">
    ...
    {Cart && Cart.length>0 && (
      <>
        <h1 className="text-5xl font-semibold text-yellow-50 mb-8">Your Cart</h1>
        {Cart.map((items, i) => (
      ...
      <div className="mt-4 w-full flex items-center justify-end">
        <div className="p-4 bg-zinc-800 rounded">
          ...
          <div className="w-[100%] mt-3">
            <button className="bg-zinc-100 rounded px-4 py-2 flex justify-denter w-full font-semibold hover:bg-zinc-200" 
              onClick={PlaceOrder}> Place your order 
            </button>
          </div>
        </div>
      </div>
